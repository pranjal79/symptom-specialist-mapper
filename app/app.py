...
.stTextArea textarea {
    ...
    padding: 1.1rem 1.3rem !important; resize: none !important;
...
.card-rank {
    ...
    letter-spacing: 0.12em; text-transform: uppercase; color: #4a5568; margin-bottom: 0.2rem;
}
.card-title { font-family: 'DM Serif Display', serif; font-size: 1.35rem; color: #f0f4ff; }
...
.confidence-bar-bg {
    ...
    border-radius: 99px; height: 6px; margin-bottom: 1.1rem; overflow: hidden;
}
...
# ── Constants ───────────────────────────────────────────────────
SPECIALIST_ICONS = {
    "Neurologist": "🧠", "Cardiologist": "❤️", "Dermatologist": "🩹",
    "Gastroenterologist": "🫁", "Pulmonologist": "🌬️", "Orthopedist": "🦴",
    "Endocrinologist": "⚗️", "Psychiatrist": "🧘", "Urologist": "💧",
    "Ophthalmologist": "👁️", "ENT Specialist": "👂", "Oncologist": "🔬",
    "Gynecologist": "🌸", "Pediatrician": "👶", "General Physician": "🩺",
}

EXAMPLES = [
    "I have severe headaches with nausea and light sensitivity",
    "Chest pain radiating to my left arm with shortness of breath",
    "Itchy red rashes spreading across my body for weeks",
...
<div class="hero">
    ...
    <p>Powered by semantic AI — not keyword search. Describe what you feel in plain language and get matched to the right doctor.</p>
</div>
...
# ── Example buttons (pill style, full text) ─────────────────────
...
# ── Text area — driven by session state ─────────────────────────
symptom_input = st.text_area(
    "Describe your symptoms",
    value=st.session_state.symptom_text,
    placeholder="e.g. I have been experiencing sharp chest pain when I breathe deeply, along with dizziness and fatigue...",
    height=130,
...
# ── Run prediction ──────────────────────────────────────────────
if st.session_state.run_prediction and st.session_state.symptom_text.strip():
    ...
    st.markdown(
        ...
        f'<span style="font-size:0.9rem;color:#4a5568;font-family:\'DM Sans\',sans-serif;">'
        f'· {inf_ms}ms</span></div>',
        unsafe_allow_html=True
    )
...
<div class="card {card_cls[i]}">
    ...
    <div class="card-match"><span>Matched pattern:</span> {matched}…</div>
...
<div class="disclaimer">
    ...
    This tool is for informational purposes only and does <strong>not</strong> constitute
    a medical diagnosis. Always consult a qualified, licensed physician for proper evaluation.
</div>
""", unsafe_allow_html=True)

    st.session_state.run_prediction = False

elif st.session_state.run_prediction and not st.session_state.symptom_text.strip():
    st.markdown('<div class="error-box">⚠️ Please describe your symptoms before searching.</div>',
                unsafe_allow_html=True)
    st.session_state.run_prediction = False