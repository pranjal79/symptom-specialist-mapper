...
def load_disease_symptom(folder_path):
    """Load primary dataset - disease symptom description"""
...
def load_symptom2disease(folder_path):
    """Load secondary dataset - symptom to disease"""
    files = [f for f in os.listdir(folder_path) if f.endswith(".csv")]
    if not files:
        raise FileNotFoundError(f"No CSV found in {folder_path}")
...
def load_medical_transcriptions(folder_path):
    """Load tertiary dataset - medical transcriptions"""
    files = [f for f in os.listdir(folder_path) if f.endswith(".csv")]
    if not files:
        raise FileNotFoundError(f"No CSV found in {folder_path}")
...
def run_ingestion():
    ...
    validate_file_exists(config["data"]["raw"]["disease_symptom"], "Disease-Symptom folder")
    validate_file_exists(config["data"]["raw"]["symptom2disease"], "Symptom2Disease folder")
    validate_file_exists(config["data"]["raw"]["medical_transcriptions"], "Medical Transcriptions folder")

    # Load all datasets
    df1 = load_disease_symptom(config["data"]["raw"]["disease_symptom"])
    df2 = load_symptom2disease(config["data"]["raw"]["symptom2disease"])
    df3 = load_medical_transcriptions(config["data"]["raw"]["medical_transcriptions"])
